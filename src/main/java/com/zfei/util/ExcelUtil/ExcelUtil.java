...
public class ExcelUtil {
     ...
     * @param out    与输出设备关联的流对象，可以将EXCEL文档导出到本地文件或者网络中
     */
    public static <T> void exportExcel(List<com.zfei.util.ExcelUtil.ExcelSheet<T>> sheets, OutputStream out) {
     ...
     * @param pattern 如果有时间数据，设定输出格式。默认为"yyy-MM-dd"
     */
    public static <T> void exportExcel(List<com.zfei.util.ExcelUtil.ExcelSheet<T>> sheets, OutputStream out, String pattern) {
        if (CollectionUtils.isEmpty(sheets)) {
            return;
        }
        // 声明一个工作薄
        HSSFWorkbook workbook = new HSSFWorkbook();
        for (com.zfei.util.ExcelUtil.ExcelSheet<T> sheet : sheets) {
            // 生成一个表格
    ...
    private static <T> void write2Sheet(HSSFSheet sheet, Map<String, String> headers, Collection<T> dataset,
        ...
        while (it.hasNext()) {
            ...
            try {
                if (t instanceof Map) {
                    @SuppressWarnings("unchecked") Map<String, Object> map = (Map<String, Object>) t;
                    int cellNum = 0;
                    //遍历列名
                    ...
                    List<com.zfei.util.ExcelUtil.FieldForSortting> fields = sortFieldByAnno(t.getClass());
                    int cellNum = 0;
    ...
    private static int setCellValue(HSSFCell cell, Object value, String pattern, int cellNum, Field field,
        ...
        if (value instanceof Integer) {
            ...
            String empty = "";
            if (field != null) {
                com.zfei.util.ExcelUtil.ExcelCell anno = field.getAnnotation(
                        com.zfei.util.ExcelUtil.ExcelCell.class);
                if (anno != null) {
     ...
     */
    public static <T> Collection<T> importExcel(Class<T> clazz, InputStream inputStream, String pattern, com.zfei.util.ExcelUtil.ExcelLogs logs, int sheetNo, int beginRow, int beginColumn,
                                                int skiprow, Integer... arrayCount) {
        Workbook workBook;
        try {
        ...
        try {
            List<com.zfei.util.ExcelUtil.ExcelLog> logList = new ArrayList<>();
            // Map<title,index>
            ...
            while (rowIterator.hasNext()) {
                ...
                //跳过哪些行
                if (row.getRowNum() == skiprow-1) {
                    continue;
                }
                if (skiprow>10&&row.getRowNum()>skiprow)
                {
                    continue;
                }
                if (row.getRowNum() == 1) {
...
//
                ...
                if (clazz == Map.class) {
                    ...
                    for (String k : titleMap.keySet()) {
                        ...
                        // 判空
                        if (cell == null) {
                            if ("State".equals(k) && currentState != null) {
                                map.put(k, currentState);
                            } else {
                                map.put(k, null);
                            }
                        } else {
                    ...
                    List<com.zfei.util.ExcelUtil.FieldForSortting> fields = sortFieldByAnno(clazz);
                    for (com.zfei.util.ExcelUtil.FieldForSortting ffs : fields) {
                        ...
                        if (field.getType().isArray()) {
                            ...
                            if (isBlank(errMsg)) {
                                ...
                                if (field.getType().equals(Date.class) && cell.getCellTypeEnum() == CellType.STRING) {
                                    ...
                                    com.zfei.util.ExcelUtil.ExcelCell annoCell = field.getAnnotation(
                                            com.zfei.util.ExcelUtil.ExcelCell.class);
                                    if (value instanceof String && !field.getType().equals(String.class) && isNotBlank(
                    ...
                    list.add(t);
                    logList.add(new com.zfei.util.ExcelUtil.ExcelLog(t, log.toString(), row.getRowNum() + 1));
                }
            }
     ...
     * @return
     */
    private static List<com.zfei.util.ExcelUtil.FieldForSortting> sortFieldByAnno(Class<?> clazz) {
        Field[] fieldsArr = clazz.getDeclaredFields();
        List<com.zfei.util.ExcelUtil.FieldForSortting> fields = new ArrayList<>();
        List<com.zfei.util.ExcelUtil.FieldForSortting> annoNullFields = new ArrayList<>();
        for (Field field : fieldsArr) {
            com.zfei.util.ExcelUtil.ExcelCell ec = field.getAnnotation(
                    com.zfei.util.ExcelUtil.ExcelCell.class);
            if (ec == null) {
            ...
            int id = ec.index();
            fields.add(new com.zfei.util.ExcelUtil.FieldForSortting(field, id));
        }
